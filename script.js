(() => {
  ...
  function tickClock() {
    ...
    renderMarketStatus(now);
  }

  function renderMarketStatus(now) {
    const minutesNow = now.getHours() * 60 + now.getMinutes();
    const openM = 9 * 60, closeM = 15 * 60 + 30;
    const isOpen = minutesNow >= openM && minutesNow < closeM;
    let remain;
    if (isOpen) remain = closeM - minutesNow;
    else remain = minutesNow < openM ? openM - minutesNow : (24 * 60 - minutesNow) + openM;
    const hh = Math.floor(remain / 60), mm = remain % 60;

    const { change } = computeNospi();
    const emergency = Math.abs(change) >= 5;

    const pill = el('statusPill');
    pill.textContent = `● ${isOpen ? '장중' : '장마감'}${emergency ? ' (비상)' : ''}`;
    pill.className = 'status-pill' + (!isOpen ? ' is-closed' : '') + (emergency ? ' is-emergency' : '');
    el('statusCountdown').textContent = `${hh}시간 ${mm}분 후 ${isOpen ? '마감' : '개장'}`;
  }

  function renderWorldTicker() {
    const track = el('tickerTrack');
    if (!state.newsLog.length) {
      track.innerHTML = '<span class="ticker-empty">시장이 개장했습니다. 소식통에서 첫 뉴스를 발행해보세요.</span>';
      return;
    }
    const items = state.newsLog.slice(0, 12).map(n => {
      ...
      return `<span class="ticker-item"><span class="tag tag-${tagClass}">${tagText}</span> <strong>${stock ? stock.name : n.ticker}</strong> ${n.headline}</span>`;
    }).join('');
  ...
  function init() {
    ...
    // Admin panel
    const adminBackdrop = el('adminModalBackdrop');
    el('btnAdmin').addEventListener('click', () => adminBackdrop.classList.add('is-open'));
    el('adminCancel').addEventListener('click', () => adminBackdrop.classList.remove('is-open'));
    adminBackdrop.addEventListener('click', e => { if (e.target.id === 'adminModalBackdrop') adminBackdrop.classList.remove('is-open'); });
    el('adminNewCompany').addEventListener('click', () => { adminBackdrop.classList.remove('is-open'); openCompanyModal('new'); });
    el('adminEditCompany').addEventListener('click', () => { adminBackdrop.classList.remove('is-open'); openCompanyModal('edit'); });
    el('adminNewNews').addEventListener('click', () => { adminBackdrop.classList.remove('is-open'); openNewsModal(); });
